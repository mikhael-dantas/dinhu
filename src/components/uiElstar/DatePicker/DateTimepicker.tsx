...
export interface DateTimepickerProps
  extends CommonProps,
    Omit<
      CalendarSharedProps,
      "onMonthChange" | "onChange" | "isDateInRange" | "isDateFirstInRange" | "isDateLastInRange" | "month"
    >,
...
const DateTimepicker = forwardRef<HTMLInputElement, DateTimepickerProps>((props, ref) => {
  ...
  const handleValueChange = (date: Date) => {
    ...
    closePickerOnChange && setInputState(capitalize(dayjs(date).locale(finalLocale).format(dateFormat)))
  ...
  const handleTimeChange = (time: Date | null) => {
    if (_value instanceof Date && time instanceof Date) {
      ...
      closePickerOnChange && setInputState(capitalize(dayjs(newDateTime).locale(finalLocale).format(dateFormat)))
    }
    closePickerOnChange && closeDropdown()
  }

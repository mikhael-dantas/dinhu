...
import type { FocusEvent, Ref, KeyboardEvent, MouseEvent, ChangeEvent } from "react"
...
const TimeInputField = forwardRef<HTMLInputElement, TimeInputFieldProps>((props, ref) => {
  const { className, onFocus, onBlur, onChange, setValue, withSeparator = false, max, min = 0, value, ...rest } = props
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "ArrowUp") {
      ...
      const padded = padTime(clamp(parseInt(event.currentTarget.value, 10) + 1, min, max as number).toString())

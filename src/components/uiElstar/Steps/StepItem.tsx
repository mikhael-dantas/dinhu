...
const StepItem = forwardRef<HTMLDivElement, StepItemProps>((props, ref) => {
  const { className, customIcon, description, isLast, onStepChange, status, stepNumber, title, vertical, ...rest } =
    props
  ...
  let stepIcon = <span>{STEP_STATUS_ICON[status as StepStatus] ?? stepNumber}</span>

  if (customIcon) {
    stepIcon = <span>{customIcon}</span>
  }

  const stepItemClass = classNames(`step-item step-item-${status}`, vertical && "step-item-vertical", className)

  const stepWrapperClass = classNames("step-item-wrapper", onStepChange && "step-clickable")
  ...
  return (
    <div className={stepItemClass} {...rest} ref={ref} role="presentation" onClick={handleStepChange}>
